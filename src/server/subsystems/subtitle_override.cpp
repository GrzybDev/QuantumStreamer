...
void SubtitleOverride::initialize(Application& app)
{
	...
	episode_names_ = app.config().getBool("Subtitles.EpisodeNames", true);
	...
	logger.information("%s append episode titles to streams", std::string(episode_names_ ? "Will" : "Will not"));
                                         ...
                                         std::map<std::string, std::vector<std::string>>& overrides)
{
	...
	if (jsonObject->has("episode_title"))
	{
		...
		m_episode_names_[epTitleKey] = jsonObject->getValue<std::string>("episode_title");
	}
                                         ...
                                         std::map<std::string, std::vector<std::string>>& overrides)
{
	...
	if (document.exists("episode_title"))
	{
		...
		const std::string epTitleKey = episode_id + "_" + trackName;
		m_episode_names_[epTitleKey] = document.get<std::string>("episode_title");
	}
                                                ...
                                                std::string& data_raw, bool is_episode_title_segment)
{
	...
	if (episode_names_ && is_episode_title_segment)
	{
		if (std::string epTitleKey = episode_id + "_" + track_name; m_episode_names_.contains(epTitleKey))
		{
			...
			AutoPtr textNode = doc->createTextNode(m_episode_names_[epTitleKey]);
			...
			if (NodeList* divList = doc->getElementsByTagName("div"); divList->length() > 0)
			{
				...
				logger.trace("Added episode title '%s' to subtitles for episode %s", m_episode_names_[epTitleKey],
				             episode_id);
			}
			else
			{
